...
async function createMO() {
    ...
    try {
        ...
        const moData = {
            ...
            Design_Eng: getCurrentUser().id, // Use user ID instead of name
        ...
        // Trigger MO creation notification
        handleMOCreation(moRecord, projectDetails);
...
async function handleMOCreation(mo, project) {
    console.log(`New MO ${mo.MO_Name} created. Triggering notifications.`);
    ...
    try {
        // Prepare list of recipients: Project Manager and Production Engineer
        const recipients = [];
        if (project && project.Project_Manager) {
            recipients.push({ id: project.Project_Manager, role: 'Project Manager' });
        }
        if (project && project.Production_Engineer) {
            recipients.push({ id: project.Production_Engineer, role: 'Production Engineer' });
        }
        
        // Filter out the sender and deduplicate recipients
        ...
        for (const recipient of uniqueRecipients) {
            const notificationData = {
              "Title": "New MO Created",
              "Message": `MO-Card #${mo.MO_Name} was created by ${currentUser.Account_Name || currentUser.Full_Name} for project ${project.Project_Name}.`,
              "Type": "MO_Creation",
              "Entity_Type": "MO-Card",
              "Entity_ID": mo.id,
            ...
            console.log(`MO creation notification created for ${recipient.role}: ${recipient.id}`);
            
            // Show browser notification if current user is the recipient
            if (recipient.id === currentUser.id) {
                console.log('Current user is recipient for MO creation, calling showBrowserNotification');
                await showBrowserNotification(notificationData);
            } else {
                console.log('Current user is not recipient for MO creation. Current user ID:', currentUser.id, 'Recipient ID:', recipient.id);
            }
        }
    } catch (error) {
        console.error('Failed to send MO creation notifications:', error);
    }
}
