    ...
    card.innerHTML = `
        <div class="card-content text-center">
            ...
            <div class="flex gap-2 justify-center">
                ...
                <button class="btn btn-success btn-sm" style="background:#25D366;color:#fff;" onclick="shareFileWhatsApp('${fileName}')">
...
async function shareAllFiles() {
    ...
    if (navigator.share) {
        try {
            ...
            console.warn('Native share failed, falling back to URLs:', err);
        }
    }

    const fileUrls = currentMO.Files.map(f => `${getPocketBaseUrl()}/api/files/MO_Table/${currentMO.id}/${f}`);
    showShareScreen(fileUrls.join('\n'), `MO Files - ${currentMO.MO_Name}`);
}
...
async function fetchFileAsFileObj(fileName) {
  ...
  // --- share a single file via native share sheet ---
  async function shareFile(fileName) {
    // require both share() and canShare() support
    if (!(navigator.share && navigator.canShare)) {
      showError('Native file sharing isn’t supported on this device/browser.');
      return;
    }
  
    try {
      const fileObj = await fetchFileAsFileObj(fileName);
  
      // only proceed if this file can actually be shared
      if (!navigator.canShare({ files: [fileObj] })) {
        showError('This file type can’t be shared natively.');
        return;
      }
  
      await navigator.share({
        title: `MO File: ${fileName}`,
        text:  `Manufacturing Order: ${currentMO.MO_Name}`,
        files: [fileObj]
      });
  
      showSuccess('File shared via native share!');
    } catch (err) {
  ...
  // --- share ALL files via native share sheet ---
  async function shareAllFiles() {
    if (!(navigator.share && navigator.canShare)) {
      showError('Native file sharing isn’t supported on this device/browser.');
      return;
    ...
    try {
      // fetch & wrap every file
      const fileObjs = await Promise.all(
        currentMO.Files.map(name => fetchFileAsFileObj(name))
      );
  
      // verify at least one is shareable
      if (!navigator.canShare({ files: fileObjs })) {
        showError('These files can’t be shared natively.');
        return;
      }
      ...
      showSuccess('All files shared via native share!');
    } catch (err) {
      console.error('shareAllFiles error:', err);
...
function shareAllFilesWhatsApp() {
    if (!currentMO || !Array.isArray(currentMO.Files) || currentMO.Files.length === 0) {
        showError('No files to share');
        return;
    }
    const fileUrls = currentMO.Files.map(file => {
        let fileName = typeof file === 'string' ? file : (file && file.name ? file.name : '');
        return `${getPocketBaseUrl()}/api/files/MO_Table/${currentMO.id}/${fileName}`;
    });
...
async function shareFileWhatsApp(fileName) {
    try {
        const fileUrl = `${getPocketBaseUrl()}/api/files/MO_Table/${currentMO.id}/${fileName}`;
        // Try advanced Web Share API with file if supported
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
            await navigator.share({
                files: [file],
                title: fileName,
                text: `File for MO: ${currentMO.MO_Name}`
            });
        } else {
            // Fallback: share link via WhatsApp
            const whatsappShareUrl = `https://wa.me/?text=${encodeURIComponent('File for MO: ' + currentMO.MO_Name + '\n' + fileUrl)}`;
            window.open(whatsappShareUrl, '_blank');
            showSuccess('Your browser does not support direct file sharing. Sharing link instead.');
        }
    } catch (err) {
        showError('Unable to share file.');
        console.error('shareFileWhatsApp error:', err);
    }
}
